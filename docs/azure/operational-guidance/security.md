...
!!! warning "Just-in-time (JIT) VM access recommendations"
    ...
    **Please ignore this specific recommendation.**
    ...
    JIT access is intended for VMs with public IP addresses. In the B.C. government Landing Zone, public IPs are blocked by guardrails. Enabling JIT will create Network Security Group (NSG) rules that can block internal RDP/SSH traffic, effectively breaking your [Azure Bastion](../tools/bastion.md) connectivity.
