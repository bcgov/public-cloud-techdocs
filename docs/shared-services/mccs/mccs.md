...
!!! danger "Testing responsibility"
    After the firewall rules have been implemented, it is **your responsibility** to test the connectivity to ensure everything is working as expected. If there are any issues identified, initiate troubleshooting by contacting [network@dxcas.com](mailto:network@dxcas.com) providing the **source**/**destination** addresses as well as the **approximate time** of the test.
