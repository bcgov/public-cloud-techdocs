...
3. **Generate the SMTP password** from the IAM user’s secret access key using AWS’s documented process:
   ...
   - SMTP password = **derived** value (not the AWS secret access key itself) - see [Obtaining Amazon SES SMTP credentials](https://docs.aws.amazon.com/ses/latest/dg/smtp-credentials.html)
